...
export default function AgentProfileCard({ profile, onDelete, onSave, isSaving = false }: AgentProfileCardProps) {
    ...
    const handleLocationConfirm = (locs: LocationSelection[]) => {
        ...
        if (selected) {
            if (selected.type !== 'WARD') {
                alert("Please select a specific Ward (Phường/Xã).");
                return;
            }
    ...
    const renderField = (label: string, icon: any, name: keyof UpdateAccountRequest, value: string | undefined | null, isLocationField: boolean = false) => {
        return (
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {isEditing ? (
                        isLocationField ? (
                            <div onClick={() => setIsLocPickerOpen(true)} className="w-full text-sm font-medium text-gray-900 border-b border-gray-300 hover:border-red-500 py-0.5 cursor-pointer flex justify-between items-center group">
                            ...
                            <input type="text" name={name} value={formData[name] as string || ''} onChange={handleInputChange} className="w-full text-sm font-medium text-gray-900 border-b border-gray-300 focus:border-red-500 focus:outline-none py-0.5" />
                        )
                    ) : (
        ...
        <>
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm relative">
                ...
                <div className="absolute top-6 right-6 flex gap-3 z-10">
                    {isEditing ? (
                        <>
                            <button onClick={handleSaveClick} disabled={isSaving} className="flex items-center gap-1.5 px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white text-xs font-bold rounded-lg transition-colors disabled:opacity-50">
                                {isSaving ? <Save className="w-3 h-3 animate-pulse" /> : <Check className="w-3 h-3" />} Confirm
                            </button>
                            <button onClick={handleCancelClick} disabled={isSaving} className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-300 text-gray-600 hover:bg-gray-50 text-xs font-bold rounded-lg transition-colors">
                                <X className="w-3 h-3" /> Cancel
                            </button>
                        </>
                    ) : (
                        <>
                            <button onClick={() => setIsEditing(true)} className="flex items-center gap-1.5 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-lg transition-colors shadow-sm">
                                <Edit className="w-4 h-4" /> Edit
                            </button>
                            <button onClick={onDelete} className="flex items-center gap-1.5 px-4 py-2 border border-red-200 text-red-600 hover:bg-red-50 text-sm font-bold rounded-lg transition-colors">
                ...
                <div className="flex flex-col md:flex-row gap-8">
                    ...
                    <div className="shrink-0">
                        <div className="w-28 h-28 rounded-full p-1 border-[3px] border-red-500 overflow-hidden relative group bg-white shadow-sm">
                            <img
                    ...
                    <div className="flex-1 pt-2">
                        <div className="mb-6 pr-32">
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                                {/* Status Badge */}
                                <Badge variant={profile.status === 'ACTIVE' ? 'success' : 'default'} className="uppercase">
                                    {profile.status}
                                </Badge>

                                <Badge variant={getTierVariant(profile.tier) as any} className="uppercase">
                                    {profile.tier || 'MEMBER'}
                                </Badge>
