...
import { PurchaseContractFilters } from '@/lib/api/services/contract.service';
...
const AsyncSelect = ({ label, value, onChange, fetchOptions, placeholder }: AsyncSelectProps) => {
    ...
    return (
        <div className="relative" ref={wrapperRef}>
            ...
            <div
                className="w-full border border-gray-300 rounded-lg px-3 py-2.5 bg-white flex items-center justify-between cursor-pointer hover:border-green-400 transition-colors shadow-sm"
                ...
                <div className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-100">
                    <div className="p-2 border-b border-gray-100 bg-gray-50">
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                className="w-full pl-8 pr-2 py-1.5 text-sm bg-white border border-gray-200 rounded-md focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-200"
                        ...
                        {loading ? (
                            ...
                            options.map(opt => (
                                <button
                                    ...
                                    className={`w-full text-left px-3 py-2 text-sm hover:bg-green-50 flex flex-col border-b border-gray-50 last:border-0 transition-colors ${value === opt.value ? 'bg-green-50 text-green-700' : 'text-gray-700'}`}
                                >
...
interface SearchProps {
    onApply: (filters: PurchaseContractFilters) => void;
    onReset: () => void;
}

export default function PurchaseContractAdvancedSearch({ onApply, onReset }: SearchProps) {
    ...
    const fetchOwners = async (keyword: string): Promise<Option[]> => {
        try {
            const res = await accountService.getAllPropertyOwners({ name: keyword, page: 1, limit: 10 });
            return res.data.map(o => ({ value: o.id, label: `${o.firstName} ${o.lastName}` }));
        } catch { return []; }
    };

    const handleApply = () => {
        const filters: PurchaseContractFilters = { page: 1, size: 10, search: '' };
    ...
    return (
        <div className="flex flex-col h-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <SelectInput
                        ...
                        options={[
                            ...
                            { label: 'Waiting Official', value: 'WAITING_OFFICIAL' },
                    ...
                    {/* Entities Group */}
                    <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5 p-5 bg-green-50/50 rounded-xl border border-green-200">
                        {/* Property - Full width for better display */}
                    ...
                    {/* Date Range - Only Start Date for Purchase */}
                    <div className="md:col-span-2">
                        <div className="space-y-3">
                            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Start Date Range</span>
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 mt-2">
                ...
                <button
                    onClick={handleApply}
                    className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-bold rounded-lg flex items-center gap-2 shadow-sm transition-colors"
                >
...
const SelectInput = ({ label, value, onChange, options }: any) => (
    <div>
        ...
        <div className="relative">
            <select
                className="w-full appearance-none border border-gray-300 rounded-lg px-3 py-2.5 bg-white text-sm focus:outline-none focus:border-green-500 cursor-pointer hover:border-gray-400 transition-colors shadow-sm"
                value={value}
...
const DateInput = ({ value, onChange, placeholder }: any) => {
    ...
    return (
        <div
            className="relative w-full border border-gray-300 rounded-lg bg-white cursor-pointer hover:border-green-500 transition-colors shadow-sm group"
        ...
        >
            <div className="flex items-center justify-between px-3 py-2.5">
                ...
                <Calendar className="w-3.5 h-3.5 text-gray-400 group-hover:text-green-500 transition-colors" />
            </div>
            <input
                ref={ref}
                type="date"
