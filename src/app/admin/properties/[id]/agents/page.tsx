    ...
    const fetchAgents = async () => {
        ...
        try {
            const [mainRes, freeRes] = await Promise.all([
                accountService.getAllSaleAgents({
                    page: currentPage,
                    limit: itemsPerPage,
                    name: searchTerm,
                    ...filters
            ...
            const mergedAgents = mainRes.data.map(agent => ({
                ...agent,
                            ...
                            {loading ? (
                                ...
                                agents.map((agent) => (
                                    <tr key={agent.id} className="hover:bg-gray-50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4"><Badge variant={agent.tier === 'PLATINUM' ? 'pink' : 'gold'}>{agent.tier || 'MEMBER'}</Badge></td>
...
// ... (Giữ nguyên component AgentAdvancedSearchForm)
